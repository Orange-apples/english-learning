...
public class BadWordUtil {
    ...
    static{
        //filePath=ClassLoader.getSystemResource("").getPath()+"敏感词库/SensitiveWord.txt";
    ...
    public static Set<String> readTxtByLine(/*String path*/){
        ...
        try (
                InputStream resourceAsStream = BadWordUtil.class.getClassLoader().getResourceAsStream("敏感词库/SensitiveWord.txt");
        ) {
    ...
    private static void addBadWordToHashMap(Set<String> keyWordSet) {
        wordMap = new HashMap(keyWordSet.size() * 2);     //初始化敏感词容器，减少扩容操作
        String key = null;
        Map nowMap = null;
...
//        System.out.println(checkBadWord("fuck",1,1));
        boolean word = BadWordUtil.isContaintBadWord("卧槽共匪", 2);
        boolean wor2 = BadWordUtil.isContaintBadWord("vddfgd", 2);
